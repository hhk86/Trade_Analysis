...
class Strategy():
    ...
    def plotSignal(self, ax: plt, n: int, xpos: int, ypos: float, sig_type: str) -> None:
    ...
    def checkCloseSignal(self, ax: plt, n: int, xpos: int, ypos: float, delta: int) -> (bool, int):
        if n > 236* 2 :
            if self.long_num > 0:
                ...
                self.closePosition("long", ax, n, xpos, ypos, 'xk')
            if self.short_num > 0:
                self.closePosition("short", ax, n, xpos, ypos, 'xk')
            return
        ...
        if self.long_num > 0:
            if delta > 0:
                ...
                if ypos < trigger_price:
                    # self.closeLongPosition(ax, n, xpos, ypos, 1)
                    self.closePosition("long", ax, n, xpos, ypos, '*k')
        ...
        if self.short_num > 0:
            if delta < 0:
                ...
                if ypos > round(trigger_price, 3):
                    ...
                    self.closePosition("short", ax, n, xpos, ypos, '*k')
    ...
    def checkOpenSignal_new(self, ax,  n: int, xpos: int, ypos: float) -> (str, str):
        if n >= 4 and n <= 234 * 2:
            if self.long_num < 1:
                if sum(self.slope_list[n - 1 : n + 1]) >= 9: # RAPB
                    self.openPosition(ax, 'B', "RAP", n, xpos, ypos, sum(self.slope_list[n - 1: n + 1]))
                else:
                    d1, d2, h1 = self.slope_list[n - 2: n + 1]
                    if d1 >= 2 and d2 >= 0 and h1 >= 2 and abs(h1) >= abs(d2) + 2: # CON1B
                        self.openPosition(ax, 'B', "CON1", n, xpos, ypos, self.slope_list[n])
            if self.short_num < 1:
                d1, d2, h1 = self.slope_list[n - 2: n + 1]
                if d1 <= -2 and d2 <= 0 and h1 <= -2 and abs(h1) >= abs(d2) + 2: # CON1S
                    self.openPosition(ax, 'S', "CON1", n, xpos, ypos, self.slope_list[n])

    def openPosition(self, ax, direction: str, sig_type: str, n: int, xpos: int, ypos:float, h1):
        if direction == 'B':
            ...
            self.short_h1 = h1
        if self.plot is True:
            self.plotSignal(ax, n, xpos, ypos, sig_type)
    ...
    def backtest_oneday(self, date: str) -> None:
        ...
        for n, xpos, ypos, delta in zip(self.Nl, self.xMl, self.yMl, self.slope_list):
            ...
            self.checkOpenSignal_new(ax, n, xpos, ypos)
        if self.plot is True:
            ax.set_xticks(self.xtick, self.xticklabel)
...
if __name__ == "__main__":
    obj=Strategy()
    # obj = Strategy(backtest_data="SH510500.csv")
    obj.multi_backtest()
    obj.printStat()
    # obj.backtest()
