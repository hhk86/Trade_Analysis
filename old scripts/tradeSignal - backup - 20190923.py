...
class Strategy():
    ...
    def checkOpenSignal(self,  n: int, xpos: int, ypos: float) -> (str, str):
        if n >= 3 and n <= 234 * 2:
            if self.long_num < 1:
                if sum(self.slope_list[n - 1 : n + 1]) >= 9: # RAPB   3
    ...
    def start(self):
        ...
        while True:
            ...
            self.xM = list(df[df["time"].apply(lambda s: s.endswith("00") or s.endswith("30"))].index)
            if len(self.xM) < 2:
                continue
            self.yM = df[df["time"].apply(lambda s: s.endswith("00") or s.endswith("30"))]["price"].tolist()
            self.xMl = self.xM[1:]
            self.yMl = self.yM[1:]
