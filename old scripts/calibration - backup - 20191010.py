...
def backtest_slice(q, name, i, x, y, z, param):
    obj = Strategy(x, y, z, name, False, param)
...
class Strategy():
    def __init__(self, x1, y, z, ticker, fake=False, param=None) -> None:
        self.ticker = ticker
        if fake is True:
            return
        try:
    ...
    def multi_backtest(self, x, y, z, process_num=10):
        ...
        for i in range(0, n):
            ...
            p = Process(target=backtest_slice, args = (q, ticker, i, x, y, z, self.multiplier))
            jobs.append(p)
            p.start()
...
if __name__ == "__main__":
    obj = Strategy(1, 1, 1, "SH510500", fake=False)
    # obj.slice()
    obj.printStat()
