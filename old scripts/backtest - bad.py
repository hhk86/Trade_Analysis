...
class Strategy():
    def __init__(self, name=None, fake=False) -> None:
        if fake is True:
            return
        if name is None:
    ...
    def defaultParam(self) -> None:
        ...
        self.P_h1_W = 0
        self.P_R_ADJ = 0
        self.switch= [True, True,    False, True, False, False, True, True,    True,True, False,True,True, False]
    ...
    def getNStyleOS(self, ls: list) -> (str, str, int):
        ...
        # if d1 <= -p4 and d2 > 0 and h1 <= -p5 and abs(d2) <= abs(d1) / 2 and abs(h1) >= 2 * abs(d2):
        #     return 'S', "NS3", h1
        if switch[6] and d1 >= 4 and d2 < 0 and h1 >= 5 and abs(d2) <= abs(d1) / 2 and abs(h1) >= 2 * abs(d2):
            return 'B', "NS3", h1
        if switch[7] and d1 <= -5 and d2 > 0 and h1 <= -4 and abs(d2) <= abs(d1) / 2 and abs(h1) >= 2 * abs(d2):
            return 'S', "NS3", h1
    ...
    def initDailyParam(self, pos_type="all", date=None) -> None:
        if pos_type == "all":
            ...
            self.yS = df["price"].tolist()[: : 10]
            self.xSl = self.xS[1:]  # x 10s list without 10s i.e. start @ 9:30:10
            self.ySl = self.yS[1:]  # price 10s list without first 10s i.e. start @ 9:30:10
            self.Ml = list(range(len(self.xSl)))
            self.slope_list_10S = [int(round(1000 * z)) for z in np.diff(self.yS)]
    ...
    def closeLongPosition_10S(self, ax: plt, n: int, xpos: float, ypos: float, sig_type: int) -> None:
        if self.plot is True:
            if sig_type == 0:
                marker = "*k"
            elif sig_type == 1:
    ...
    def closeShortPosition_10S(self, ax: plt, n: int, xpos: float, ypos: float, sig_type: int) -> None:
        if self.plot is True:
            if sig_type == 0:
                marker = "*k"
            elif sig_type == 1:
    ...
    def checkCloseLongSignal_10S(self, n: int, ypos: float, delta: int) -> (bool, int):
        if n > 236:
            return True, 1, n * 6
        for m in range(6 * n - 60, 6 * n, 10):
            delta = self.slope_list_10S[m]
            ypos = self.ySl[m]
            if delta > 0:
                if round(1000 * (ypos - self.long_start_price)) >= 6:
                    self.long_reach_6 = True
    ...
    def checkCloseShortSignal_10S(self, n: int, ypos: float, delta: int) -> (bool, int):
        if n > 236:
            return True, 1, n * 6
        for m in range(6 * n - 60, 6 * n, 10):
            delta = self.slope_list_10S[m]
    ...
    def backtest_oneday(self, date: str) -> None:
        ...
        for n, xpos, ypos, delta in zip(self.Nl, self.xMl, self.yMl, self.slope_list):
            ...
            if self.long_num > 0:
                sig, close_type, m = self.checkCloseLongSignal_10S(n, ypos, delta)
                if sig is True:
                    self.closeLongPosition(ax, m, self.xSl[m], self.ySl[m], close_type)
            if self.short_num > 0:
                sig, close_type, m = self.checkCloseShortSignal_10S(n, ypos, delta)
                if sig is True:
                    self.closeShortPosition(ax, m, self.xSl[m], self.ySl[m], close_type)
    ...
    def printStat(self, name=None, printSave=True) -> None:
        ...
        if printSave is True:
            ...
            if name is None:
                ...
                self.stat.to_csv("calibration/stat_"+ name +".csv")
        return self.stat
    ...
    def calibrate(self) -> None:
        ...
        # obj.printStat("P_NS_1_is_4")
        # self.defaultParam()
        # self.P_NS_1 = 5
        # self.backtest(plot=False)
        # obj.printStat("P_NS_1_is_5")
        #
        # # NS2
        # self.defaultParam()
        # self.P_NS_2 = 6
        # self.P_NS_3 = 4
        # self.backtest(plot=False)
        # obj.printStat("P_NS_2_3_is_6_4")
        # self.defaultParam()
        # self.P_NS_2 = 6
        # self.P_NS_3 = 5
        # self.backtest(plot=False)
        # obj.printStat("P_NS_2_3_is_6_5")
        # self.defaultParam()
        # self.P_NS_2 = 6
        # self.P_NS_3 = 6
        # self.backtest(plot=False)
        # obj.printStat("P_NS_2_3_is_6_6")
        # self.defaultParam()
        # self.P_NS_2 = 7
        # self.P_NS_3 = 4
        # self.backtest(plot=False)
        # obj.printStat("P_NS_2_3_is_7_4")
        # self.defaultParam()
        # self.P_NS_2 = 7
        # self.P_NS_3 = 5
        # self.backtest(plot=False)
        # obj.printStat("P_NS_2_3_is_7_5")
        # self.defaultParam()
        # self.P_NS_2 = 7
        # self.P_NS_3 = 6
        # self.backtest(plot=False)
        # obj.printStat("P_NS_2_3_is_7_6")
        #
        # #NS3
        # self.defaultParam()
        # self.P_NS_4 = 3
        # self.P_NS_5 = 5
        # self.backtest(plot=False)
        # obj.printStat("P_NS_4_5_is_3_5")
        # self.defaultParam()
        # self.P_NS_4 = 3
        # self.P_NS_5 = 6
        # self.backtest(plot=False)
        # obj.printStat("P_NS_4_5_is_3_6")
        # self.defaultParam()
        # self.P_NS_4 = 4
        # self.P_NS_5 = 4
        # self.backtest(plot=False)
        # obj.printStat("P_NS_4_5_is_4_4")
        # self.defaultParam()
        # self.P_NS_4 = 4
        # self.P_NS_5 = 5
        # self.backtest(plot=False)
        # obj.printStat("P_NS_4_5_is_4_5")
        # self.defaultParam()
        # self.P_NS_4 = 4
        # self.P_NS_5 = 6
        # self.backtest(plot=False)
        # obj.printStat("P_NS_4_5_is_4_6")
        # self.defaultParam()
        # self.P_NS_4 = 5
        # self.P_NS_5 = 4
        # self.backtest(plot=False)
        # obj.printStat("P_NS_4_5_is_5_4")
        # self.defaultParam()
        # self.P_NS_4 = 5
        # self.P_NS_5 = 5
        # self.backtest(plot=False)
        # obj.printStat("P_NS_4_5_is_5_5")
        # self.defaultParam()
        # self.P_NS_4 = 5
        # self.P_NS_5 = 6
        # self.backtest(plot=False)
        # obj.printStat("P_NS_4_5_is_5_6")
        #
        #
        # # CON1
        # self.defaultParam()
        # self.P_CON_1 = 4
        # self.backtest(plot=False)
        # obj.printStat("P_CON_1_is_4")
        # self.defaultParam()
        # self.P_CON_1 = 5
        # self.backtest(plot=False)
        # obj.printStat("P_CON_1_is_5")
        #
        # # CON2
        # self.defaultParam()
        # self.P_CON_2 = 6
        # self.P_CON_3 = 4
        # self.backtest(plot=False)
        # obj.printStat("P_CON_2_3_is_6_4")
        # self.defaultParam()
        # self.P_CON_2 = 6
        # self.P_CON_3 = 5
        # self.backtest(plot=False)
        # obj.printStat("P_CON_2_3_is_6_5")
        # self.defaultParam()
        # self.P_CON_2 = 6
        # self.P_CON_3 = 6
        # self.backtest(plot=False)
        # obj.printStat("P_CON_2_3_is_6_6")
        # self.defaultParam()
        # self.P_CON_2 = 7
        # self.P_CON_3 = 4
        # self.backtest(plot=False)
        # obj.printStat("P_CON_2_3_is_7_4")
        # self.defaultParam()
        # self.P_CON_2 = 7
        # self.P_CON_3 = 5
        # self.backtest(plot=False)
        # obj.printStat("P_CON_2_3_is_7_5")
        # self.defaultParam()
        # self.P_CON_2 = 7
        # self.P_CON_3 = 6
        # self.backtest(plot=False)
        # obj.printStat("P_CON_2_3_is_7_6")
        #
        # #CON3
        # self.defaultParam()
        # self.P_CON_4 = 3
        # self.P_CON_5 = 5
        # self.backtest(plot=False)
        # obj.printStat("P_CON_4_5_is_3_5")
        # self.defaultParam()
        # self.P_CON_4 = 3
        # self.P_CON_5 = 6
        # self.backtest(plot=False)
        # obj.printStat("P_CON_4_5_is_3_6")
        # self.defaultParam()
        # self.P_CON_4 = 4
        # self.P_CON_5 = 4
        # self.backtest(plot=False)
        # obj.printStat("P_CON_4_5_is_4_4")
        # self.defaultParam()
        # self.P_CON_4 = 4
        # self.P_CON_5 = 5
        # self.backtest(plot=False)
        # obj.printStat("P_CON_4_5_is_4_5")
        # self.defaultParam()
        # self.P_CON_4 = 4
        # self.P_CON_5 = 6
        # self.backtest(plot=False)
        # obj.printStat("P_CON_4_5_is_4_6")
        # self.defaultParam()
        # self.P_CON_4 = 5
        # self.P_CON_5 = 4
        # self.backtest(plot=False)
        # obj.printStat("P_CON_4_5_is_5_4")
        # self.defaultParam()
        # self.P_CON_4 = 5
        # self.P_CON_5 = 5
        # self.backtest(plot=False)
        # obj.printStat("P_CON_4_5_is_5_5")
        # self.defaultParam()
        # self.P_CON_4 = 5
        # self.P_CON_5 = 6
        # self.backtest(plot=False)
        # obj.printStat("P_CON_4_5_is_5_6")
        #
        #
        #
        #
        ...
        # for P_h1_W in [0.33, 0.5, 0.67]:
        # for P_h1_W in [0, ]:
        #     for (P_W_1, P_W_2) in zip([0.6, 0.7, 0.75], [0.7, 0.8, 0.85]):
        #         for (P_L_1, P_L_2) in zip([12, 18], [20, 40]):
        #             self.defaultParam()
        #             self.P_h1_W = P_h1_W
        #             self.P_W_1 = P_W_1
        #             self.P_W_2 = P_W_2
        ...
        #             self.P_L_2 = P_L_2
        #             self.backtest(plot=False)
        #             obj.printStat("P_h1w_" + str(P_h1_W) + "_w1_" + str(P_W_1) + "_w2_" + str(P_W_2)\
        #                           + "_h1_" + str(P_L_1) + "_h2_" + str(P_L_2))
        #
        # for P_h1_W in [0, 0.33, 0.5, 0.67]:
        #     for (P_W_1, P_W_2) in zip([0.67, ], [0.75,]):
        #         for (P_L_1, P_L_2) in zip([15,], [30,]):
        #             self.defaultParam()
        #             self.P_h1_W = P_h1_W
        #             self.P_W_1 = P_W_1
        #             self.P_W_2 = P_W_2
    ...
    def multi_backtest(self, process_num=10):
        ...
        for i in range(0, n):
            ...
            p = Process(target=backtest_slice, args = (q, name, i))
            jobs.append(p)
            p.start()
